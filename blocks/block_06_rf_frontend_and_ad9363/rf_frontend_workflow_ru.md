...
| Debug waveform format | помогает найти сигнал в эфире и проверить тракт по известным признакам |
...
| Expected preamble/sync | `1010...` / `0xA5A55A5A` | проверка обнаружения пакета |
...
## Отладочный сигнал для поиска в эфире

Для RF bring-up полезно передавать не произвольный поток, а специальный отладочный кадр:

```text
silence → lead-in tone → preamble → sync word → header → training → payload/PRBS → CRC → silence
```

Минимальные режимы, которые стоит поддержать в TX:

- pure tone — найти передачу и проверить частотный план;
- preamble only — проверить корреляционный поиск;
- repeated sync word — проверить порядок битов/байтов;
- PRBS packet — посчитать BER;
- amplitude sweep — найти перегруз и подобрать gain/attenuator;
...
| Пакеты видны пачками на waterfall | frame period и silence работают корректно |
| Корреляция по преамбуле даёт устойчивый пик | сигнал найден автоматически |
| Sync word найден после преамбулы | порядок битов/байтов согласован |
...
| Корреляционный пик есть, но CRC не проходит | ошибка timing/CFO/bit order | проверить sync, training и демодулятор |
...
5. описание debug waveform или `tx_mode`;
6. скриншот или FFT-график нормального режима;
7. результат поиска preamble/sync;
8. признаки перегрузки или их отсутствие;
9. IQ metadata file;
10. инженерный вывод.
