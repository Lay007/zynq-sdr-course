...
The estimator is now validated end to end on silicon across its full operating range, which is the
precondition for wiring it into the fabric receiver. `qpsk_rx_bit_recovery_chain` already
instantiates it ahead of the Costas loop behind a runtime `coarse_cfo_en` bit (held 0 today); the
[Block 05 smoke suite](../block_05_fpga_hdl_flow/) covers the integrated path — the coherent
loopback stays bit-identical with the estimator bypassed, and a 25 kHz injected offset it removes
to BER 0. The remaining on-silicon step is to drive `coarse_cfo_en` from a `gp_ctrl` bit through
the bridge and re-run the two-board link with the offset acquired **in fabric** rather than on the
host.
