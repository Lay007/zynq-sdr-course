...
| QPSK replay dataset | `datasets/demo_qpsk_capture/`, `tools/generate_demo_qpsk_dataset.py`, `tools/analyze_demo_qpsk_dataset.py` | Public 64 KiB Git LFS synthetic fixture with analyzer workflow | A legally clean deterministic QPSK CI16 dataset can be downloaded, replayed, regenerated byte-for-byte and checked for SHA256, BER/SER, EVM and CFO | Keep analyzer thresholds and report text synchronized with generated metrics |
