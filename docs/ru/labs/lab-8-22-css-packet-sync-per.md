...
Результаты по пакетам в зависимости от SNR (1000 пакетов на точку, введён CFO 1,25 бина):

| SNR | Пропуск обнаружения | PER без коррекции | PER с коррекцией | SER с коррекцией |
|---:|---:|---:|---:|---:|
| −15 дБ | 0,948 | 1,0 | 1,0 | 0,609 |
| −12 дБ | 0,419 | 1,0 | 0,93 | 0,139 |
| −9 дБ | 0,013 | 1,0 | 0,045 | 0,004 |
| −6 дБ | 0 | 1,0 | 0 | 0 |
| −3 дБ | 0 | 1,0 | 0 | 0 |

Смещение частоты дискретизации (200 пакетов на точку): до 100 ppm ничего не отказывает; на 250 ppm отказывает каждый пакет без коррекции, а коррекция восстанавливает все; на 500 ppm даже приёмник с коррекцией теряет 20 % пакетов. В 1000 испытаниях только с шумом ложных срабатываний нет.

- **Без коррекции CFO не проходит ничего ни при каком SNR**: сдвиг 1,25 бина смещает каждый символ на один бин плюс четверть — ровно тот отказ, что предсказывает Лабораторная 8.21.
- **Ниже −9 дБ доминирует пропуск обнаружения.** Пакет теряется раньше, чем его увидит детектор полезной нагрузки, поэтому PER растёт быстрее SER.

## Упражнения

1. Какова 95-процентная верхняя граница PER при −6 дБ, если в 1000 пакетах 0 отказов? Что нужно, чтобы заявить PER ниже 1e-4?
2. Переведите 250 ppm смещения частоты дискретизации в дрейф 16-символьной полезной нагрузки (в отсчётах) к её последнему символу. Почему даже приёмник с коррекцией начинает терять пакеты на 500 ppm?
