...
При `SF = 7`, `BW = 125 кГц`: 128 чипов на символ, 1,024 мс на символ, ошибка постоянства огибающей на уровне точности плавающей запятой (2e-16). Символ 37 после сжатия чирпа попадает в **бин FFT 37**, а исходный восходящий чирп имеет пик в бине 0. В этом случае без шума отношение пика ко второму (192 дБ) отражает лишь численную точность; что с ним делает шум, показывает Лабораторная 8.21.

## Упражнения

1. Замените `SF` на 9. Сколько чипов и сколько миллисекунд на символ? Сколько бит несёт один символ?
2. Закодируйте символы 127 и 0. Куда они попадают после сжатия чирпа и почему соседние символы лежат в соседних бинах?
3. Умножьте принятый чирп на небольшой частотный сдвиг (четверть бина). Что происходит с бином пика и его высотой?
