...
Анализатор на закоммиченном наборе данных сообщает `evm_rms_percent = 0.00332`, `evm_peak_percent = 0.00332`, `snr_estimate_db = 89.59`, `mean_i_normalized = 0.0214` и оценку CFO порядка 1e-14 Гц.

1. Выведите EVM из амплитуды CI16. Каждая компонента QPSK равна 12000 / sqrt(2) = 8485,28 и хранится как 8485. Покажите, что ошибка округления даёт ровно 0,00332 % и что 20 log10(1 / 3,32e-5) — это оценка SNR 89,6 дБ.
2. Почему пиковый EVM равен среднеквадратичному? (Все четыре точки созвездия округляются одинаково.) Чего вы ожидаете на реальной записи?
3. `mean_i_normalized` равно 0,0214, а не 0. Это смещение постоянной составляющей? Посчитайте символы +I и -I в полезной нагрузке и объясните значение; затем опишите, как отличить дисбаланс данных от настоящего смещения DC приёмника.
4. Оценка CFO находится на уровне шума вычислений с плавающей точкой и меняется между запусками в последних знаках. Почему анализатор реальных записей должен сообщать её с указанным разрешением, а не печатать как есть?
