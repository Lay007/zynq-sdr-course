...
Два из этих чисел означают не то, чем кажутся:

- **EVM 0,14 % — это не округление Q1.15.** 24-битный шаг фазы даёт -120 000,0286 Гц вместо -120 000 Гц. За 32 768 отсчётов (13,7 мс) эта ошибка 0,0286 Гц накапливает около 0,0025 рад фазы относительно идеального эталона с плавающей точкой, и этот дрейф и есть EVM. При `phase_bits = 32` EVM падает до 0,0027 %. Спектр этого не покажет: 0,0286 Гц — глубоко внутри одного FFT-бина шириной 73 Гц, поэтому «Frequency shift error: 0.000 Hz».
- **«Спур» -70,12 дБc — это самый высокий бин шума**, а не паразитная составляющая смесителя. Вход содержит `noise_rms = 0.01`. При `noise_rms = 0` наибольший спур равен -98,2 дБc — реальный предел Q1.15 для этого смесителя.

### Упражнения

1. Установите `phase_bits = 16`. Ошибка частоты становится -7,32 Гц, EVM — 35,9 %, при этом наибольший спур по-прежнему -69,1 дБc, а пик всё так же попадает в бин 0 Гц. Объясните, почему проверка только по спектру приняла бы такой NCO и что измеряет EVM относительно эталона, чего не измеряет спектр.
2. Установите `noise_rms = 0` и `q_fractional_bits = 11`. Наибольший спур растёт с -98,2 до -79,6 дБc. Близко ли это к 6 дБ на бит? Какую часть смесителя (слова данных или слова синуса/косинуса) вы бы расширили в первую очередь?
3. Сколько бит фазы нужно, чтобы при 2,4 MS/s накопленная ошибка фазы сдвига оставалась ниже 0,01 рад на пакете длительностью 100 мс?
