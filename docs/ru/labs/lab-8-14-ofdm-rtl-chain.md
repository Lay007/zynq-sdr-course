...
- что преобразования работают на 100 МГц. Out-of-context реализация в Vivado 2021.1 на `xc7z020clg400-2` ([отчёт](https://github.com/Lay007/zynq-sdr-course/blob/main/reports/fpga/block8-ofdm-vivado-evidence_ru.md)) разводит каждый OFDM-блок без ошибок DRC, и эквалайзер, трекер пилотов и удаление CP легко укладываются в 100 МГц. Но `ofdm_tx_cp16_path` и `ofdm_fft64_sequential` не укладываются примерно на 10 нс (WNS после трассировки -10,2 / -10,8 нс, около 49 / 48 МГц, примерно по 8,8 тыс. LUT): общая бабочка умножает, округляет, насыщает и обновляет счётчик насыщений за один такт (28 уровней логики), а 64-точечная рабочая память лежит в логике, а не в блочной RAM. Конвейеризация бабочки меняет расписание IFFT и пока не сделана. Комментарии в заголовках блоков указывают латентность в тактах (например, у IFFT `384 compute clocks after input collection`).
...
- [x] Отчёт по ресурсам и таймингу по OOC-реализации в Vivado (все блоки разведены; пути IFFT/FFT не укладываются в 100 МГц).
- [ ] Конвейерная бабочка и рабочая память в блочной RAM, укладывающиеся в 100 МГц.
