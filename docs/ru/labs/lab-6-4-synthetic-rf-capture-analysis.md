...
- **Ошибка 12,2 Гц — квантование по бинам FFT**, а не RF-ошибка: анализ использует FFT на 65 536 точек, поэтому при 2,4 МС/с бины отстоят на 36,6 Гц, и пик читается в ближайшем бине. На реальном захвате тот же анализ добавил бы к этому сдвиг генераторов передатчика и приёмника.
- **«SNR 74 дБ» — это пик над медианным бином FFT**, поэтому он включает выигрыш обработки FFT на длинной записи. Это не SNR на отсчёт и не сравнимо с SNR, вычисленным по EVM. Всегда указывайте определение.
- **Уровень пика −5,8 дБFS при нуле ограничений** означает, что синтетический захват использует примерно половину полной шкалы; на железе по этому числу выставляют усиление RX.

### Упражнения

1. Задайте в метаданных `processing.fft_length` = 16 384 и перезапустите. Как меняются частотная ошибка и «SNR» и почему SNR меняется, хотя сигнал нет?
2. Поднимайте амплитуду тона, пока счётчик ограничений не станет ненулевым. Что происходит со спектром (гармоники, поднятая полка)?
3. Запишите ожидаемое смещение для реального захвата, где LO TX 915,000 МГц, цифровой тон +100 кГц, а LO RX 914,950 МГц.
