...
1. При 8 дБ таблица показывает 2,2e-4 против теоретических 1,91e-4. Переведите оба значения в число ошибок на 400 000 бит (88 и около 76) и решите, значима ли разница, используя пуассоновское стандартное отклонение (примерно корень из числа ошибок).
2. Чему равно Es/N0 для QPSK при Eb/N0 = 6 дБ? Объясните, почему BER на бит всё равно равен BPSK при том же Eb/N0, хотя при той же энергии символа точки QPSK расположены ближе друг к другу.
3. Замените отображение Грея натуральным (дибиты 00, 01, 10, 11 в соседних квадрантах по порядку). Предскажите отношение BER до запуска. Быстрое моделирование этой замены дало примерно 1,4–1,5 от BER Грея при 0, 4 и 8 дБ (например, 1,89e-2 против 1,27e-2 при 4 дБ): при натуральном отображении один из двух соседних квадрантов отличается в обоих битах, поэтому средняя ошибка символа стоит 1,5 бита вместо 1.
4. HDL-loopback печатает `PASS: qpsk_zynq_ber_top loopback recovered 140 QPSK symbols at BER=0 (start_offset=62)`. Сколько это бит и какой наименьший BER может заявить этот единственный прогон?
