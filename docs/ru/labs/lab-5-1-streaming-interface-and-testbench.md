...
Testbench подаёт 8 векторов, два из них с `in_valid = 0`, включая полномасштабные значения +32767 и -32768. После сброса он проверяет каждый такт: `out_valid` должен совпадать с `in_valid`, задержанным на один такт, а `out_i`/`out_q` — с задержанным входом, если задержанный valid равен 1. На пустых тактах данные не сравниваются: по контракту они там «не важны».
...
1. Нарушьте контракт valid: замените `out_valid <= in_valid;` на `out_valid <= 1'b1;`. Testbench сообщает `ERROR at 45000: out_valid=1 expected=0` на каждом пустом такте и завершается `FAIL: iq_passthrough test completed with 5 errors`.
2. Уберите `if (in_valid)` вокруг регистров данных, чтобы `out_i`/`out_q` загружались каждый такт. Testbench всё равно **проходит**. Объясните, почему это корректно для контракта только с valid, и назовите ситуацию, где удержание последнего валидного отсчёта всё же оправдывает clock enable (следующий блок игнорирует `valid`, waveform легче читать, мощность переключений).
3. Добавьте вторую ступень регистров (задержка 2). До правки testbench предскажите, какая проверка сработает первой; затем измените задержку эталонной модели, чтобы testbench снова проходил. Запишите новую задержку в отчёт.
