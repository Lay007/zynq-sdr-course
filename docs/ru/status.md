...
| 05 | FPGA / HDL flow | Ready | Executable | Verilog testbenches + AXI-Lite-controlled Zynq-ready BPSK BER top-level + gpreg-based AD9361 overlay scaffolding + integrated CLG400 bitstream/XSA + live proof that both rebuilt vendor ZIP shells still diverge from the stock BOOT.bin PL image | Ready | Hardware pending | HDL CI | Сохранить извлечённый PL из штатного `BOOT.bin` как единственный подтвержденный boot-shell, а затем возвращать course RTL постепенно, не заменяя этот baseline. |
...
| 11 | Интегрированный SDR-проект | Ready | Executable | Simulation package + BPSK reference package + AXI-Lite helper + gpreg overlay tooling + historical `done+timeout` bring-up evidence + overlap contention probe + extracted stock BOOT.bin PL baseline that clean-boots with AD9361 and IIO intact | Ready | Hardware pending | Labs | Вернуть sample-domain gpreg bridge строго вокруг точного stock-shell, заново подтвердить clean boot и только потом продолжать RF discovery к первым ненулевым `RECEIVED_BITS`. |
...
| `docs/assets/lab112_clean_boot_pl_validation.json` | Постоянный артефакт с доказательством, что извлеченный PL из `BOOT.bin` clean-boot проходит, а оба пересобранных vendor ZIP bitstream нет. |
