...
| 05 | FPGA / HDL flow | Measured signoff candidate | 29 HDL-тестов; рабочий snapshot полностью разведен, strategy sweep выбрал WNS +0,096 нс, fabric-loopback на плате проходит | Проверить устойчивость выбранной реализации на повторных сборках/seeds. |
...
| 11 | Интегрированный SDR-проект | Timing-closed coarse-CFO; измерен residual-CFO hardening | Lab 11.32 захватывает 12/12 CFO-точек. Lab 11.33 принимает Costas-фикс по сохранённому захвату (124/280 → 0/280 ошибок), измеряет 27/40 чистых живых попыток при +30 кГц и отвергает timing-closed `I²+Q²` picker, лучший равнобюджетный прогон которого дал лишь 14/40. | Реализовать непрерывный QPSK timing recovery, затем снять длинную BER-статистику и BER-vs-аттенюация. |
