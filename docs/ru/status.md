...
| 11 | Интегрированный SDR-проект | Ready | Executable | Simulation package + BPSK reference package + AXI-Lite helper + gpreg overlay tooling + historical `done+timeout` bring-up evidence + overlap contention probe + raw-clean-boot доказательство того, что кандидат `bridge_txrx_mux` действительно доходит до PL и открывает `axi_gpreg` по адресу `0x79040000`, хотя AD9361 все еще уходит в timeout, плюс manual-UART `fpga load` evidence того, что stock BOOT partition сейчас остается единственным boot-safe external PL payload | Ready | Hardware pending | Labs | Воспроизвести boot-safe свойства stock partition в source-correlated или editable shell, а затем продолжать RF discovery к первым ненулевым `RECEIVED_BITS`. |
...
| `hardware/7020_ad936x_sdr/boot/build_system_bit_bin.py` | Воспроизводимый Bootgen-wrapper для runtime Linux / packaged-image payloads в формате `.bit.bin` и для ручных экспериментов с U-Boot `fpga load`. |
