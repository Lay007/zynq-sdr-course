...
| 11 | Интегрированный SDR-проект | Коарс-CFO оценщик в составе fabric-битстрима; обе платы грузят курсовую конфигурацию и гоняют его в фабрике | Коарс-CFO (`qpsk_coarse_cfo.v`, `COARSE_ENABLE=1`) закрыт по таймингу (такт 16 нс, +7,997 нс); ОБЕ платы грузят курсовой битстрим на boot через u-boot `fpga loadb` и дают in-fabric fabric-loopback BER=0 (280 бит, 0 ошибок); оценщик проверен на реальном межплатном CFO двух плат (Lab 11.30/11.31, бит-в-бит совпадающая fixed-point RTL-модель, SER 0/28672) | Путь непрерывного TX, чтобы in-fabric RX платы B декодировал живой кадр платы A при BER=0 — двухплатный fabric-линк установлен (B ловит кадры A), чистый декод требует host-fed непрерывного fabric-кадра. |
