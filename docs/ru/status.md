...
| 05 | FPGA / HDL flow | Measured signoff candidate | Canonical suite содержит 34 HDL-теста. Payload-telemetry image полностью разведён при WNS +0,009 нс / WHS +0,028 нс, чисто загружается на board B и выдаёт live preamble/payload split. | Добавить payload error-position telemetry, затем проверить repeat-build/seed stability. |
...
| 11 | Интегрированный SDR-проект | Парный continuous timing измерен; baseline сохранён | Lab 11.35 записывает 1200 пар, диагностирует axis-sign TED, отвергает dot-product replacement по focused clean gate, затем в 160-парном instrumentation run локализует все 55 однобитовых промахов Gardner в payload, а не preamble. Gardner повышает lock до 146/160 и снижает payload BER до 0,073523, но даёт лишь 4/160 clean attempts против 44/160 fixed. | Записать first/last либо segmented payload error positions и отличить within-frame drift от равномерной потери decision margin. |
