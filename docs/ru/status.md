...
| 05 | FPGA / HDL flow | Measured signoff candidate | 33 HDL-теста; рабочий snapshot полностью разведён при WNS +0,096 нс, а QPSK Gardner из Lab 11.34 закрывает timing при WNS +0,049 нс / WHS +0,031 нс и покрыт bit-exact model/RTL и runtime-mux тестами | Проверить образ Gardner на платах, затем проверить repeat-build/seed stability. |
...
| 11 | Интегрированный SDR-проект | Timing-closed coarse-CFO и continuous timing candidate | Lab 11.32 захватывает 12/12 CFO-точек, Lab 11.33 сохраняет Costas-фикс и отвергает худший live picker. Lab 11.34 добавляет runtime-selectable QPSK Gardner: 0/12 000 ошибок модели при ±100 ppm, bit-exact RTL, 0/280 на drifted chain, retained-capture coverage и routed WNS +0,049 нс. | Выполнить живой A/B Gardner против baseline, затем снять длинную BER и BER-vs-аттенюация. |
