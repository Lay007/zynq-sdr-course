...
| Блок | Тема | Текущее состояние | Уровень доказательств | Основной пробел / следующее улучшение |
|---|---|---|---|---|
| 01 | Введение в SDR | Ready | Реальные RTL-SDR записи и управляемый DDS-тон Zynq | Добавить короткий учебный отчёт, сравнивающий пассивный захват и управляемый тон. |
...
| 05 | FPGA / HDL flow | Measured signoff candidate | Canonical suite содержит 36 HDL-тестов. Payload-telemetry image полностью разведён при WNS +0,009 нс / WHS +0,028 нс, чисто загружается на board B и выдаёт live preamble/payload split; отгружаемый differential-QPSK образ (текущий модем board B) закрывается при WNS +0,036 нс. | Проверить repeat-build/seed timing robustness разведённого образа. |
...
| 08 | Модуляция и синхронизация | Executable + RTL baseline | CFO/timing, QPSK, 16-QAM, GFSK, DSSS, OFDM и packet CSS models; синтезируемая цепочка OFDM (mapper, IFFT/FFT, циклический префикс, одноотводный эквалайзер), побитово проверенная относительно моделей Q1.15 (Lab 8.14, issue #48); реальные BPSK-метрики на железе (Lab 8.15); bit-exact SF7 dechirp/sequential-detector RTL-регрессия по всем символам | AXI-обёртка OFDM и слежение за фазой по пилотам в RTL пока не реализованы (issue #48). Компактный CSS-детектор остаётся учебным; SF5–SF12 LoRa PHY, interoperability, oversampled correlator, packet/ToA-тракт и интеграция с платой развиваются в [zynq-lora-phy-positioning](https://github.com/Lay007/zynq-lora-phy-positioning). |
...
У каждой страницы лабораторной в Блоках 1–10 и 12 есть полная русская версия. В Блоке 11 два самых объёмных журнала аппаратных экспериментов (Lab 11.8 и 11.26) в русской редакции имеют русское краткое изложение в начале, а подробная хронология пока только на английском; каждая такая страница сообщает об этом и ссылается на [карту лабораторных Блока 11](blocks/11-integrated-sdr-project.md), где также сказано, какие работы выполнять, а какие читать как разборы случаев.
...
5. ✔ Финальный отчёт готов: Lab 11.4 связывает model → HDL → RF → метрики → инженерный вывод, с pass/fail и воспроизводимостью.
