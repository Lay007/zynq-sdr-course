...
| 11 | Интегрированный SDR-проект | Коарс-CFO в составе fabric-битстрима; ДВУХПЛАТНЫЙ in-fabric коарс-CFO проверен на реальном межплатном CFO | Коарс-CFO (`qpsk_coarse_cfo.v`, `COARSE_ENABLE=1`) закрыт по таймингу (такт 16 нс, +7,997 нс); обе платы грузят курсовой битстрим на boot через u-boot `fpga loadb` и дают in-fabric fabric-loopback BER=0; IN-FABRIC оценщик (`gp_ctrl[13]`) декодирует живой кадр по кабелю от второй платы при BER 0/280 во всём диапазоне 0–55 кГц инжектированного межплатного CFO, тогда как одна петля Костаса падает в каждой точке (119–144 ошибок, нет захвата на 55 кГц) — плата A (вендор Pluto) шлёт бит-в-бит курсовой кадр, плата B (курсовой) fabric-RX+coarse декодирует | Длинные cabled/OTA BER-статистики и кривая BER-vs-аттенюация на двухплатном линке. |
