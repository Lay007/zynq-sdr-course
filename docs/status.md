...
| 11 | Integrated SDR project | Coarse-CFO estimator shipped in the fabric bitstream; both boards boot the course config and run it in-fabric | Coarse-CFO (`qpsk_coarse_cfo.v`, `COARSE_ENABLE=1`) is timing-closed (16 ns clock, +7.997 ns); BOTH boards load the course bitstream at boot via u-boot `fpga loadb` and reach in-fabric fabric-loopback BER=0 (280 bits, 0 errors); the estimator is validated on a real two-board inter-board CFO (Lab 11.30/11.31, bit-identical fixed-point RTL model, SER 0/28672) | Continuous-TX path so board B's in-fabric RX decodes board A's live frame at BER=0 -- the two-board fabric link is established (B captures A's frames), clean decode needs a host-fed continuous fabric frame. |
