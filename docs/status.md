...
| 05 | FPGA / HDL flow | Measured signoff candidate | 33 HDL tests; hardware-working snapshot is fully routed at WNS +0.096 ns, and the Lab 11.34 QPSK Gardner candidate closes at WNS +0.049 ns / WHS +0.031 ns with bit-exact model/RTL and runtime-mux coverage | Verify the Gardner image on hardware, then challenge repeat-build/seed stability. |
...
| 11 | Integrated SDR project | Timing-closed coarse-CFO shipped; continuous timing candidate timing-closed | Lab 11.32 acquires all 12 CFO points over 0–55 kHz. Lab 11.33 retains the Costas fix and rejects a worse live picker. Lab 11.34 adds a runtime-selectable QPSK Gardner loop: 0/12,000 model errors at ±100 ppm, bit-exact RTL, 0/280 drifted-chain errors, retained-capture coverage and routed WNS +0.049 ns. | Run the live baseline/candidate A/B, then measure longer cabled/OTA BER and BER-vs-attenuation. |
