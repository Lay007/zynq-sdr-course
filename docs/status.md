...
| 05 | FPGA / HDL flow | Ready | Executable | Verilog testbenches + AXI-Lite-controlled Zynq-ready BPSK BER top-level + gpreg-based AD9361 overlay scaffolding + integrated CLG400 bitstream/XSA + live proof that the standalone no-OS reference `system_top.bit` clean-boots when converted to `system.bit.bin`, while the rebuilt vendor ZIP shells and the pure-Tcl `vendor_only` recreation still diverge from that baseline | Ready | Hardware pending | HDL CI | Promote the validated no-OS reference `system_top.bit`/XSA pair to the course PL baseline, then graft the course overlay around that handoff because the pure-Tcl `vendor_only` recreation still leaves read-only or disabled deltas. |
...
| 11 | Integrated SDR project | Ready | Executable | Simulation package + BPSK reference package + AXI-Lite helper + gpreg overlay tooling + historical `done+timeout` bring-up evidence + overlap contention probe + validated clean-boot baselines from both the extracted stock BOOT.bin PL partition and the standalone no-OS reference `system_top.bit` | Ready | Hardware pending | Labs | Reattach the sample-domain gpreg bridge around the validated no-OS reference shell, revalidate clean boot, and only then resume RF discovery for the first non-zero `RECEIVED_BITS`. |
...
| `hardware/7020_ad936x_sdr/boot/extract_stock_system_top_partition.py` | Reproducible extractor for the known-good `system_top.bit` PL partition embedded in `boot/sd_image/BOOT.bin`. |
| `hardware/7020_ad936x_sdr/boot/build_system_bit_bin.py` | Reproducible Bootgen wrapper for converting a raw `.bit` into the `system.bit.bin` payload expected by the clean boot overlay. |
| `docs/assets/lab112_clean_boot_pl_validation.json` | Persistent evidence that both the extracted BOOT.bin PL partition and the bootgen-converted no-OS reference `system_top.bit` clean-boot, while the regenerated vendor ZIP bitstreams do not. |
| `hardware/7020_ad936x_sdr/ps/ad936x_no_os_reference/platform/hw/system_top.bit` | Source-correlated vendor reference bitstream that now clean-boots when converted to `system.bit.bin`. |
...
| `docs/assets/vendor_reference_vs_vendor_xpr_snapshot_handoff_diff.json` | Persistent diff showing that the saved vendor `zc702.xpr` snapshot is the closest surviving source witness: it differs from the vendor reference XSA only at `sys_ps7` MIO14/15 direction fields. |
