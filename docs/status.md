...
| 05 | FPGA / HDL flow | Ready | Executable | Verilog testbenches + AXI-Lite-controlled Zynq-ready BPSK BER top-level + gpreg-based AD9361 overlay scaffolding + integrated CLG400 bitstream/XSA + live proof that the rebuilt vendor shells from both vendor ZIP paths still diverge from the stock BOOT.bin PL image | Ready | Hardware pending | HDL CI | Keep the extracted stock BOOT.bin PL as the only validated boot shell, then reintroduce course RTL incrementally without replacing that baseline. |
...
| 11 | Integrated SDR project | Ready | Executable | Simulation package + BPSK reference package + AXI-Lite helper + gpreg overlay tooling + historical `done+timeout` bring-up evidence + overlap contention probe + extracted stock BOOT.bin PL baseline that clean-boots with AD9361 and IIO intact | Ready | Hardware pending | Labs | Reattach the sample-domain gpreg bridge around the exact stock shell, revalidate clean boot, and only then resume RF discovery for the first non-zero `RECEIVED_BITS`. |
...
| `docs/assets/lab112_clean_boot_pl_validation.json` | Persistent evidence that the extracted BOOT.bin PL partition clean-boots, while the two regenerated vendor ZIP bitstreams do not. |
