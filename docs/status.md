...
| 05 | FPGA / HDL flow | Measured signoff candidate | 29 HDL tests; hardware-working snapshot is fully routed, strategy sweep selects WNS +0.096 ns, and passes board fabric-loopback qualification | Verify repeat-build/seed stability for the selected implementation. |
...
| 11 | Integrated SDR project | Timing-closed coarse-CFO shipped; residual-CFO hardening measured | Lab 11.32 acquires all 12 CFO points over 0–55 kHz. Lab 11.33 accepts a retained-capture Costas fix (124/280 to 0/280 errors), measures 27/40 clean live attempts at +30 kHz, and rejects a timing-closed squared-energy picker whose best equal-budget run reached only 14/40. | Implement continuous QPSK timing recovery, then measure longer cabled/OTA BER and BER-vs-attenuation. |
