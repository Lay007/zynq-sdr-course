...
| 08 | Modulation and synchronization | Executable + RTL baseline | CFO/timing, QPSK, 16-QAM, GFSK, DSSS, OFDM and packet CSS models; a synthesizable OFDM chain (mapper, IFFT/FFT, cyclic prefix, one-tap equalizer) verified bit-exactly against Q1.15 models (Lab 8.14, issue #48); real-hardware BPSK metrics (Lab 8.15); a bit-exact SF7 dechirp/sequential-detector RTL regression over all symbols | OFDM AXI wrapper and RTL pilot-phase tracking are not built yet (issue #48). The compact CSS detector is educational; SF5–SF12 LoRa PHY, interoperability, oversampled correlation, packet/ToA logic and board integration remain in [zynq-lora-phy-positioning](https://github.com/Lay007/zynq-lora-phy-positioning). |
...
Every lab page in Blocks 1-10 and Block 12 has a full Russian version. In Block 11, the two
longest hardware-experiment logs (Labs 11.8 and 11.26) have a Russian summary at the top and an
English-only detailed chronology in the Russian edition; each such page says so and links to the
