...
| 11 | Integrated SDR project | Coarse-CFO shipped in the fabric bitstream; in-fabric TWO-BOARD coarse-CFO validated on a real inter-board CFO | Coarse-CFO (`qpsk_coarse_cfo.v`, `COARSE_ENABLE=1`) is timing-closed (16 ns clock, +7.997 ns); both boards load the course bitstream at boot via u-boot `fpga loadb` and reach in-fabric fabric-loopback BER=0; the IN-FABRIC estimator (`gp_ctrl[13]`) decodes a live over-cable frame from a second board at BER 0/280 across 0-55 kHz injected inter-board CFO, while Costas-only fails at every point (119-144 errors, no lock at 55 kHz) -- board A (vendor Pluto) streams the bit-exact course frame, board B (course) fabric-RX+coarse decodes | Longer cabled/OTA BER statistics and a BER-vs-attenuation curve on the two-board link. |
