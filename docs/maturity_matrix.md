...
| 05 | FPGA / HDL flow | measured / signoff-target | 33 HDL tests plus a timing-clean, board-qualified vendor-snapshot bitstream selected at WNS +0.096 ns; the Lab 11.34 QPSK Gardner candidate closes at WNS +0.049 ns / WHS +0.031 ns with bit-exact vectors and runtime-mux coverage | Verify Gardner on hardware, then demonstrate repeat-build/seed timing robustness. |
...
| 11 | Integrated SDR project | measured / portfolio-target | Lab 11.32 acquires 12/12 CFO points; Lab 11.33 retains a Costas pull-in fix and rejects a timing-closed picker in live A/B; Lab 11.34 implements continuous Gardner recovery and qualifies it in float/fixed/RTL, retained data and physical timing at WNS +0.049 ns | Complete the controlled live A/B, then collect long-duration BER-vs-attenuation statistics. |
